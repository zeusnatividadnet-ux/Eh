...
function handleScroll() {
    ...
    // Parallax effect for blobs (only on desktop for performance)
...
function createParticles() {
    ...
    const particleCount = isMobile ? 15 : 40; // Fewer particles on mobile for performance
...
// Only create particles if device has good performance
...
window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(() => {
        // Recalculate particle count if needed
        const existingParticles = document.querySelectorAll('.particle');
        const isMobile = window.innerWidth <= 768;
        const targetCount = isMobile ? 15 : 40;
        
        if (existingParticles.length > targetCount) {
            // Remove excess particles on resize to mobile
...
if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    ...
    document.querySelectorAll('.gradient-blob').forEach(b => b.style.animation = 'none');
}
