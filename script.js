...
document.getElementById('closeMessageBtn').addEventListener('click', closeMessageOverlay);
...
// ===================================
let isRoadmapActive = false;

function showRoadmap() {
    isRoadmapActive = true;
    const portfolioContainer = document.getElementById('portfolioContainer');
    const roadmapContainer = document.getElementById('roadmapContainer');
    const navItemsPortfolio = document.querySelectorAll('.nav-item-portfolio');
    const navItemRoadmap = document.querySelector('.nav-item-roadmap');
    const navLinkRoadmap = document.querySelector('.nav-link-roadmap');
    
    // Hide portfolio with animation
    portfolioContainer.style.opacity = '0';
    portfolioContainer.style.transform = 'translateX(-100px)';
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            roadmapContainer.classList.add('active');
            createStars();
            animateRoadmapItems();
            animateRoadmapStats();
        }, 50);
    ...
    // Update navigation
    navItemsPortfolio.forEach(item => item.style.display = 'none');
    navItemRoadmap.style.display = 'block';
    navLinkRoadmap.style.display = 'none';
...
function showPortfolio() {
    ...
    // Hide roadmap with animation
    roadmapContainer.classList.remove('active');
    
    setTimeout(() => {
        roadmapContainer.style.display = 'none';
        portfolioContainer.style.display = 'block';
        
        // Show portfolio with animation
        setTimeout(() => {
    ...
    // Update navigation
    navItemsPortfolio.forEach(item => item.style.display = 'block');
    navItemRoadmap.style.display = 'none';
    navLinkRoadmap.style.display = 'block';
...
// Event listeners for roadmap navigation
document.querySelector('.nav-link-roadmap').addEventListener('click', (e) => {
    e.preventDefault();
    showRoadmap();
});
...
function createStars() {
    const starsContainer = document.querySelector('.roadmap-stars');
...
function animateRoadmapItems() {
    ...
    document.querySelectorAll('.reveal-roadmap').forEach(el => {
        roadmapObserver.observe(el);
    });
}

// Animate roadmap statistics
function animateRoadmapStats() {
    const statsObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
    ...
    const statsContainer = document.querySelector('.roadmap-stats');
    if (statsContainer) {
